...
@Daniel
"""
def genROSOldrad(radData,cells,etype):
    import numpy as np
    ...
    def get_yields(etype):
        if etype == 1:  # electron
            return 0.7, 2.5 # Adjust when literature is available
        elif etype == 2:  # proton
            return 0.7, 2.5 #Adjust when literature is available
        elif etype == 3:  # Fe-56 
            #return 3.5, 7.0
    ...
    fe56_hits = 0  # Counter for Fe-56 hits

    for i in range(n):
        radPos = [radData[i,0],radData[i,1],radData[i,2]]
        energy = radData[i,3]
        etype = int(radData[i,4])
        G_H2O2,G_OH = get_yields(etype)
        yield_H2O2 = G_H2O2*energy
        yield_OH = G_OH*energy
        cellHit = 0  # reset hit flag for each radiation event
        for c in cells:
            currPos = c.position
            if radPos[0] <= currPos[0]+2 and radPos[0] >= currPos[0]-2:
                if radPos[1] <= currPos[1]+2 and radPos[1] >= currPos[1]-2:
                    if radPos[2] <= currPos[2]+2 and radPos[2] >= currPos[2]-2:
                        cellHit = 1
                    if etype == 3:  # Fe-56
                        print(f"[HIT] Fe-56 hit cell at {currPos} with radPos {radPos}")
                        fe56_hits += 1
