...
class Cell:
    ...
    def cellROS(self,g,radGen,ROSData):
        ...
        # ROSData[0:3]###############################3
        # # pandas =>
        # # iloc[ROSDAta[0] is near currPos] ->
        # # iloc[Previousiloc[1] is near currPos] ->t
        # #
        # namess = ["Generation", "x", "y", "z", "Health"]
        namess = ["Posx", "Posy", "Posz", "C_H2O2", "C_OH", "cellHit"]
        # if iloc empty: break cell is healthy >>> <<<< ignore ROS damage (need to check every step of iloc)
        # ###############
        dp = pd.DataFrame(ROSData, columns = namess)
        # Healthy = results.loc[results["Health"] == 1]
        dp = dp.loc[((dp["Posx"] <= currPos[0] + 2) & (dp["Posx"] >= currPos[0] - 2))]
        ...
        if dp.empty == False:
            ...
            dp = dp.reset_index()
            OH_C = dp['C_OH'].to_numpy()
            H2O2_C = dp['C_H2O2'].to_numpy()
            # maybe dp = dp.reset_index(drop=True)
...
# Load the data
        apopDat = np.genfromtxt(file_path, skip_header=1)
